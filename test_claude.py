"""LLM evaluation harness for numbergame using Anthropic Claude."""
...
def call_llm(
    ...
    """Call Anthropic Claude with structured output and retry on rate limits.
    ...
    if reasoning_effort:
        ...
        budget = effort_to_budget.get(reasoning_effort, 5000)
        kwargs["thinking"] = {
            "type": "enabled",
    ...
    for attempt in range(max_retries):
        try:
            ...
            if action is None:
                raise ValueError("API returned None for parsed_output")

            # Extract thinking summaries if present
            if reasoning_effort and hasattr(response, "content"):
                thinking_summaries = []
                ...
                if thinking_summaries:
                    thinking_summary = " ".join(thinking_summaries)
                    action.reasoning = (
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run LLM evaluation on numbergame with Claude"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output JSONL file path (default: runs/{timestamp}_claude.jsonl)",
    )
    ...
    # Support both naming conventions and dated versions
    if not any(model == m or model.startswith(f"{m}-") for m in supported_models):
        raise ValueError(
            f"Unsupported Claude model: {model}. "
            f"Supported models: claude-opus-4-5, claude-sonnet-4-5 "
            f"(and dated versions)"
        )
    ...
    if args.output:
        ...
        output_path = runs_dir / f"{timestamp}_claude.jsonl"
